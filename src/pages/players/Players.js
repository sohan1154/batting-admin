...
class Players extends React.Component {
    ...
    changeBettingStatus = (id, status) => {

        let _this = this;

        // document.body.classList.add('display-loader');

        GlobalProvider.confirmBox("Are you Sure? You want to change batting status of this account.", (isTrue) => {
            if (isTrue) {
                ApisService.changePlayerBettingStatus(id, status)
                    .then(response => {
                                                                        ...
                                                                        <span className="changeBettingStatus" onClick={() => this.changeBettingStatus(item.id, !item.is_betting)}>
                                                                            {item.is_betting ? (
                                                                                <button type="button" className="btn btn-sm m-b-15 ml-2 mr-2 btn-rounded-circle btn-success" title="Locked"><i className="mdi mdi-check"></i></button>
                                                                                    ...
                                                                                    <button type="button" className="btn btn-sm m-b-15 ml-2 mr-2 btn-rounded-circle btn-warning" title="Open"><i className="mdi mdi-close"></i></button>
                                                                                )}
                                                                        </span>
                                                                    </td>
                                                                    <td>
